# This Makefile is for the AudioFile::Info::Audio::WMA extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/AudioFile/Info/Audio/WMA.pm]
#     AUTHOR => [q[Markus <holli.holzer@googlemail.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[AudioFile::Info::Audio::WMA]
#     PL_FILES => {  }
#     PREREQ_PM => { Audio::WMA=>q[0], Test::More=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/AudioFile/Info/Audio/WMA.pm]
#     clean => { FILES=>q[AudioFile-Info-Audio-WMA-*] }
...
NAME_SYM = AudioFile_Info_Audio_WMA
VERSION = 0.11
...
MAKE = make
FULLEXT = AudioFile/Info/Audio/WMA
BASEEXT = WMA
PARENT_NAME = AudioFile::Info::Audio
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/AudioFile/Info/Audio/WMA.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/AudioFile/Info/Audio
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/AudioFile/Info/Audio
...
TO_INST_PM = lib/AudioFile/Info/Audio/WMA.pm

PM_TO_BLIB = lib/AudioFile/Info/Audio/WMA.pm \
	blib/lib/AudioFile/Info/Audio/WMA.pm
...
DISTNAME = AudioFile-Info-Audio-WMA
DISTVNAME = AudioFile-Info-Audio-WMA-0.11
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/AudioFile/Info/Audio/WMA.pm $(INST_MAN3DIR)/AudioFile::Info::Audio::WMA.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Perl extension to get info from WMA files.'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Markus <holli.holzer@googlemail.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: AudioFile-Info-Audio-WMA' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '  Audio::WMA: '\''0'\''' >> META_new.yml
	$(NOECHO) $(ECHO) '  Test::More: '\''0'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: 0.11' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "Perl extension to get info from WMA files.",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Markus <holli.holzer@googlemail.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "AudioFile-Info-Audio-WMA",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '         "requires" : {' >> META_new.json
	$(NOECHO) $(ECHO) '            "Audio::WMA" : "0",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl extension to get info from WMA files.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Markus &lt;holli.holzer@googlemail.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Audio::WMA" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/AudioFile/Info/Audio/WMA.pm blib/lib/AudioFile/Info/Audio/WMA.pm 
